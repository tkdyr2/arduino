#include <Wire.h>
#include <OzOLED.h>


const static unsigned char Eduino_Logo[] PROGMEM ={



};

byte Brightness = 0;

void setup(){

	OzOled.init();  //initialze Eduino OLED display

	OzOled.setInverseDisplay();          // Set inverse display
	OzOled.setPageMode();                // Set display addressing to page mode.
	OzOled.clearDisplay();               // clear the screen and set start position to top left corner
	OzOled.drawBitmap(Eduino_Logo, 0, 0, 16, 8);

	delay(2000);
}

void loop(){

	OzOled.setBrightness(Brightness++);  // Change the brightness 0 - 255
	
	OzOled.setCursorXY(7,13);              // Set the Cursor position to 7th Page , 13th Column
	OzOled.printNumber((long)Brightness);       
	// or you can use 
	// OzOled.printNumber(Brightness, 7, 13);
	
	delay(50);                              // Delay 50ms between different brightness number
	
	if(Brightness >= 255) {
		Brightness = 0;                      // Reset Brighness to 0.
		OzOled.printString("   ", 7, 13);    // Clear 13,14 and 15th Columns. As the numbers are always left aligned - 2 digit and 3 digit will overwrite.
	}
}

